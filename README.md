Here is a comprehensive `README.md` file for your project. You can copy and paste this directly into your repository.
...
Current "State-of-the-Art" Multimodal Fusion models are computationally expensive and battery-hungry, making them impractical for continuous use on wearable devices. Simple cascaded models (Text $\rightarrow$ Audio) are efficient but unreliable because they fail to detect context gaps (e.g., a polite phrase spoken in a threatening tone).

### Our Solution: The Triage Cascade

We propose a hybrid architecture that balances **high reliability** with **high efficiency**:

1.  **Tier 1 (The Triage Layer):** Runs lightweight Text and Audio models in parallel to filter clear-cut cases.
2.  **Tier 2 (Reliability Engine):** Checks for contextual consistency. If signals match (e.g., Safe Text + Safe Audio), it outputs a decision immediately.
3.  **Tier 3 (The Specialist):** Only escalates to the expensive Video/Fusion model if a **Context Mismatch** or ambiguity is detected.
...
| :--- | :--- | :--- | :--- |
| **Tier 1** | **Gatekeeper** | **Text:** DistilRoBERTa (Intent)<br>**Audio:** Wav2Vec2 (Emotion) | 100% of interactions |
